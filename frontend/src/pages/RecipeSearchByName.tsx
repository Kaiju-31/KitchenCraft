...
export default function RecipeSearchByName() {
  ...
  const { data: recipes, loading, error, refetch } = useAPICache<Recipe[]>(
    name ? `/api/recipes/by-name?name=${encodeURIComponent(name)}${scaledPerson ? `&scaledPerson=${scaledPerson}` : ''}` : null,
    async () => {
      if (!name) return [];
      return await recipeService.searchRecipesByName(name, scaledPerson);
    },
    {
  ...
  // Redirection si pas de nom dans l'URL
  useEffect(() => {
    if (!name) {
      navigate('/recipes');
    }
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-bold text-slate-800 flex items-center gap-3">
            ...
            Recherche: "{decodeURIComponent(name || '')}"
          </h1>
      ...
      <div className="space-y-4">
        ...
        {!recipes || recipes.length === 0 ? (
          <EmptyState
            ...
            description={`Aucune recette ne correspond à "${decodeURIComponent(name || '')}"`}
